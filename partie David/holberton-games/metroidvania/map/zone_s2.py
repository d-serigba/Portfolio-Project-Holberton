# ============================================================
#  ARNAUD METROIDVANIA — zone_s2.py
#  Salle de sauvegarde — accessible depuis Zone 1
# ============================================================
...
class ZoneS2(ZoneBase):
    ...
    def _construire(self):
        ...
        self.sorties = [
            {"cote": "droite", "y": H-T*4, "taille": T*2,
             "destination": "zone_1", "spawn": "depuis_zs2"},
        ]

        # Plateforme avec le drapeau
        ...
        self.drapeau_rect = pygame.Rect(
            self.drapeau_x - T, self.drapeau_y, T*2, T*2
        )
    ...
    def verifier_save(self, joueur, state_manager):
        T = TAILLE_TUILE
        """
        Appelée par la Facade chaque frame.
        Si Arnaud touche le drapeau, active la sauvegarde.
        """
        if joueur.rect.colliderect(self.drapeau_rect) and self.cooldown_save == 0:
            self.deja_active   = True
            self.cooldown_save = 600  # 10 secondes a 60 FPS
    ...
    def dessiner(self, ecran, cam_x, cam_y):
        ...
        # Drapeau — pulse vert si actif, gris si pas encore touché
        if self.deja_active:
