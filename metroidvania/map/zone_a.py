...
class ZoneA:
    ...
    def __init__(self):
        ...
        # Point de spawn du joueur — bas droite
        self.spawns = {"defaut": (self.LARGEUR_MONDE - 3 * TAILLE_TUILE,
                      self.HAUTEUR_MONDE - 3 * TAILLE_TUILE)}
    ...
    def _construire(self):
        ...
        # ── SOL PRINCIPAL ──────────────────────────────────────
        # Sol continu tout en bas sauf un puits au centre
        # Gauche du puits
        Plateforme(0, H - T*2, W//2 - T*2, T*2).add(self.plateformes)
        # Droite du puits (spawn)
        Plateforme(W//2 + T*2, H - T*2, W//2 - T*2, T*2).add(self.plateformes)

        # ── MURS EXTÉRIEURS ────────────────────────────────────
        Mur(0,     0,      T*2, H).add(self.murs)         # mur gauche
        Mur(W-T*2, 0,      T*2, H).add(self.murs)         # mur droit
        Plateforme(0, 0, W, T*2, GRIS_MUR).add(self.plateformes)  # plafond

        # ── PUITS CENTRAL ──────────────────────────────────────
        # Les murs du puits (joueur peut tomber dedans)
        cx = W // 2
        Mur(cx - T*2, H - T*8, T*2, T*6).add(self.murs)   # mur gauche puits
        Mur(cx + T*0, H - T*8, T*2, T*6).add(self.murs)   # mur droit puits
        # Fond du puits (plus bas = mort zone ou récupération)
        Plateforme(cx - T*2, H - T*2, T*4, T*2, (40, 40, 50)).add(self.plateformes)

        # ── PLATEFORME HAUTE GAUCHE — zone secrète ─────────────
        # Accessible uniquement avec double saut (débloqué plus tard)
        px_secret = T * 5
        py_secret = H - T * 10
        Plateforme(px_secret, py_secret, T * 8, T * 2).add(self.plateformes)
        # Légo caché dessus
        Lego(px_secret + T*3, py_secret - T, couleur_idx=0).add(self.legos)

        # ── BORDURE HAUTE — apprend le SAUT ────────────────────
        # Posée sur le sol, entre le spawn et le centre
        bx = W - T * 15
        by = H - T * 2 - T * 3   # posée sur le sol
        BordureHaute(bx, by).add(self.murs)

        # Indication visuelle : flèche dessinée sur le sol avant la bordure
        # (simple tuile colorée légèrement différente)
        Plateforme(bx - T*4, H - T*2, T*4, T, (80, 85, 95)).add(self.plateformes)

        # ── PLATEFORME INTERMÉDIAIRE — après la bordure ─────────
        # Permet de souffler après le saut
        Plateforme(bx - T*10, H - T*5, T*6, T).add(self.plateformes)

        # ── ENNEMI BASIQUE — apprend l'ATK ─────────────────────
        # Posé sur la plateforme intermédiaire
        ex = bx - T*8
        ey = H - T*5 - 32
        lim_g = bx - T*10
        lim_d = bx - T*4
        EnnemBasique(ex, ey, lim_g, lim_d).add(self.ennemis)

        # ── PLATEFORME AVANT TOURELLE ───────────────────────────
        Plateforme(T*8, H - T*2, T*14, T*2).add(self.plateformes)

        # ── TOURELLE — apprend l'ESQUIVE ───────────────────────
        # Fixée sur un mur à gauche, tire vers la droite (direction 1)
        tx = T * 10
        ty = H - T * 2 - T   # au-dessus du sol
        Tourelle(tx, ty, direction=1).add(self.tourelles)

        # ── CHECKPOINT S ────────────────────────────────────────
        # Après la tourelle, avant la sortie
        cp = Checkpoint(T * 6, H - T * 2 - T * 2)
        self.checkpoints.append(cp)

        # ── SORTIE — portail de fin de zone ─────────────────────
        # Mur gauche avec ouverture = passage vers zone suivante
        # (géré dans main.py : si joueur.rect.left <= 0 → changer zone)
